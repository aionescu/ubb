...
public class MaintainerController {
    ...
    Set<MaintainerDto> getAllMaintainers(){
        log.trace("getAllMaintainers - method entered");
        var toReturn = maintainerConverter.convertModelsToDtos(maintainerService.getAllMaintainers());
        log.trace("getAllMaintainers - result={}", toReturn);
        return toReturn;
    }

    @RequestMapping(value = "/maintainers/filter/type={type}&input={input}", method = RequestMethod.GET)
    Set<MaintainerDto> getFilteredMaintainers(@PathVariable String type, @PathVariable String input){
        log.trace("getFilteredMaintainers - method entered");
        var toReturn = maintainerConverter.convertModelsToDtos(maintainerService.getFilteredMaintainers(type, input));
