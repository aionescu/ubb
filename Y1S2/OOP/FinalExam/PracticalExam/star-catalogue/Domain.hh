...
public:
  enum { DatumCount = sizeof...(TFields) };
...
class Astronomer : public Entity<Astronomer, std::string, std::string> {
public:
  using Entity::Entity;

  enum { name, constellation };
};

class Star : public Entity<Star, std::string, std::string, int, int, int> {
public:
  using Entity::Entity;

  enum { name, constellation, rightAscension, declination, diameter };
};
