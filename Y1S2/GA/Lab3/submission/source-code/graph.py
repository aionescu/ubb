...
class Graph:
  ...
  def __printMatrix(self, matrix: List[List[int]]) -> None:
    infify = lambda x: "∞" if x == INF else str(x)

    for row in matrix[1:]:
      print("[" + ", ".join(map(infify, row[1:])) + "]")

    print()

  def floydWarshall(self, v1: int, v2: int, printIntermediate: bool = False) -> Optional[Tuple[int, List[int]]]:      
    self.__assertVertexExists(v1)
    self.__assertVertexExists(v2)
    ...
    for i in range(1, n + 1):
      for j in range(1, n + 1):
        if i == j:
          d[i][j] = 0
        elif self.existsEdge(i, j):
          d[i][j] = self.getCost(i, j)
        else:
          d[i][j] = INF

    if printIntermediate:
      print("D[0]:")
      self.__printMatrix(d)

    # Initialize the previous matrix `p`
    ...
    for k in range(1, n + 1):
      ...
      if printIntermediate:
        print(f"k = {k} -> Using vertex #{k} as intermediate vertex.\n")
